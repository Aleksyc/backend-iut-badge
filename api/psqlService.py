        ...
        if params:
            for i, (key, value) in enumerate(params.items(), start=1):
                if value != "":
                    ...
                    elif key == "datetime_pres_start": value = datetime.datetime.strptime(value, "%Y-%m-%d").date(); query += f" AND datetime_pres::date >= ${i}"
                    elif key == "datetime_pres_end": value = datetime.datetime.strptime(value, "%Y-%m-%d").date(); query += f" AND datetime_pres::date <= ${i}"
